...
class UIButton(UIElement):
    ...
    def _reload_graphics(self) -> None:
        ...
        text_surf = self._manager.ressources["fonts"]["regular"].render(self._text, True, "white")
        blit_pos = pygame.Vector2(self._rect.size) * 0.5 - pygame.Vector2(text_surf.get_size()) * 0.5
        
        for state, color in self._manager.ressources["button_images"].items():
    ...
    def handle_event(self, event: pygame.Event) -> None:
        if event.type == pygame.MOUSEBUTTONDOWN:
            pos = self.get_inner_position_from(pygame.Vector2(event.pos)) * POSITION_SCALE_FACTOR
