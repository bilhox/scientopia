...
def generate_texture(settings: dict) -> pygame.Surface:
    ...
    noise_values1 = noise1.noise2array(
        numpy.array([3 * i / size for i in range(size)]),
        numpy.array([3 * i / size for i in range(size)]),
    )
...
for setting in settings:
    if settings[setting]["type"] == "ranged value":
        ui_settings[setting] = RangedParameter(
            pygame.Rect(0, 10 + i * 32, 550, 30),
            manager=manager,
            container=menu,
            name=setting,
        ...
        ui_settings[setting] = DetailParameter(
            pygame.Rect(0, 10 + i * 32, 550, 30),
            manager=manager,
            container=menu,
            name=setting,
...
generate_button = pygame_gui.elements.UIButton(
    pygame.Rect(20, 20, 200, 40), text="generate", manager=manager
)
...
while is_running:
    ...
    window_surface.blit(
        result, pygame.Vector2(300, 350) - pygame.Vector2(result.get_size()) / 2
    )
